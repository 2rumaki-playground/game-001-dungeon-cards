...
3. 各PRについて [コンフリクトの確認](#ref-コンフリクトの確認) の手順でmainブランチとのコンフリクト有無を確認する
...
また、[コンフリクトの確認](#ref-コンフリクトの確認) の手順でmainブランチとのコンフリクト有無を確認する。
...
mainブランチとのコンフリクトがある場合は、その旨も合わせて提示し、レビューコメント対応と併せてコンフリクト解消も行うか確認する。
...
PRがmainブランチとコンフリクトしているかを確認する:

```
gh pr view <番号> --json mergeable -q .mergeable
```
...
- 依存関係が未インストールの場合は、コンフリクト解消後に `pnpm install` を実行してから次の手順に進むこと
- その上で、`pnpm build` と `pnpm test:run` で動作確認を行うこと
