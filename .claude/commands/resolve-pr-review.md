...
gh api graphql -f query='
  ...
  } }' --jq '.data.repository.pullRequest.reviewThreads.nodes[] | select(.isResolved == false) | {id, comments: [.comments.nodes[] | {databaseId, body: (.body | split("\n")[0][:120])}]}'
```
...
未解決スレッドのコメント全文が必要な場合は、REST APIで個別取得する:
...
コメント(databaseId) 列には返信に使用するコメントの `databaseId`、スレッドID 列には解決に使用するスレッドの `id` を記載すること。
