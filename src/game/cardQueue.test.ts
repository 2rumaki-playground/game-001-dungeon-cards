...
describe("cardQueue", () => {
	describe("canEnqueueCard", () => {
		it("未使用カードは予約可能", () => {
			const card: Card = { id: "c1", type: "move", level: 1, exp: 0 };
		...
		it("キュー内に別のカードがあれば予約可能", () => {
			const card1: Card = { id: "c1", type: "move", level: 1, exp: 0 };
			const card2: Card = { id: "c2", type: "attack", level: 1, exp: 0 };
			const queue: QueuedCard[] = [{ card: card1, direction: "up" }];
			expect(canEnqueueCard(queue, card2)).toBe(true);
		});

		it("deck未指定でも未使用カードは予約可能", () => {
			const card: Card = { id: "c1", type: "wait", level: 1, exp: 0 };
			expect(canEnqueueCard([], card)).toBe(true);
		});
	});
	...
	describe("buildQueuedCardIndexMap", () => {
		...
		it("1件のキューは1始まりのインデックスを返す", () => {
			const queue: QueuedCard[] = [
				{ card: { id: "c1", type: "move", level: 1, exp: 0 }, direction: "up" },
			];
		...
		it("複数件のキューは挿入順にインデックスを返す", () => {
			const queue: QueuedCard[] = [
				{ card: { id: "c1", type: "move", level: 1, exp: 0 }, direction: "up" },
				{
					card: { id: "c2", type: "attack", level: 1, exp: 0 },
					direction: "down",
				},
				{
					card: { id: "c3", type: "jump", level: 1, exp: 0 },
					direction: "left",
				},
			];
